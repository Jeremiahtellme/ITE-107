...
public class PhonebookSystem extends JFrame {
    ...
    private JButton addButton, deleteButton, searchButton, updateButton, displayButton, addGroupButton, deleteGroupButton, editGroupButton;
    ...
    private void editGroup() {
        String oldGroupName = groupField.getText();
        String newGroupName = JOptionPane.showInputDialog(this, "Enter new group name:", "Edit Group", JOptionPane.PLAIN_MESSAGE);

        if (oldGroupName.isEmpty() || newGroupName == null || newGroupName.isEmpty() || !phonebook.containsKey(oldGroupName)) {
            JOptionPane.showMessageDialog(this, "Invalid group name.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        if (phonebook.containsKey(newGroupName)) {
            JOptionPane.showMessageDialog(this, "New group name already exists.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        phonebook.put(newGroupName, phonebook.remove(oldGroupName));
        contactArea.append("Group renamed from " + oldGroupName + " to " + newGroupName + "\n");
        saveDataToFile();
    }
